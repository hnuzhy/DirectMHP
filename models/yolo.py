...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True, num_angles=0):  # detection layer
        super().__init__()
...
class Model(nn.Module):
    ...
    def forward(self, x, augment=False, profile=False, visualize=False, scales=[0.5, 1, 2], flips=[None, 3, None]):
        if augment:
            return self.forward_augment(x, s=scales, f=flips)  # augmented inference, None
        return self.forward_once(x, profile, visualize)  # single-scale inference, train

    def forward_augment(self, x, s=[0.5, 1, 2], f=[None, 3, None]):
