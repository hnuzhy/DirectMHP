...
def plot_pose_cube(img, yaw, pitch, roll, tdx=None, tdy=None, size=100.):
...
def plot_3axis_Zaxis_by_euler_angles(img, yaw, pitch, roll, tdx=None, tdy=None, size=50., limited=True):
    # Input is a cv2 image
    # pose_params: (pitch, yaw, roll, tdx, tdy)
    # Where (tdx, tdy) is the translation of the face.
    # For pose we have [pitch yaw roll tdx tdy tdz scale_factor]
    ...
    cv2.line(img, (int(tdx), int(tdy)), (int(endx), int(endy)), (0,255,255), 2)


    cv2.line(img, (int(face_x), int(face_y)), (int(x1),int(y1)),(0,0,255),2)  # X-Axis pointing to right. drawn in red
    cv2.line(img, (int(face_x), int(face_y)), (int(x2),int(y2)),(0,255,0),2)  # Y-Axis pointing to down. drawn in green
    cv2.line(img, (int(face_x), int(face_y)), (int(x3),int(y3)),(255,0,0),2)  # Z-Axis (out of the screen) drawn in blue
    
    
    return img


def plot_cuboid_Zaxis_by_euler_angles(img, yaw, pitch, roll, tdx=None, tdy=None, size=100., limited=True):
    # Input is a cv2 image
    # pose_params: (pitch, yaw, roll, tdx, tdy)
    # Where (tdx, tdy) is the translation of the face.
    # For pose we have [pitch yaw roll tdx tdy tdz scale_factor]
