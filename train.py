...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors'), num_angles=num_angles).to(device)  # create
        ...
        model = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors'), num_angles=num_angles).to(device)  # create
    ...
    for epoch in range(start_epoch, epochs):
        ...
        LOGGER.info(('\n' + '%10s' * 8) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'mse', 'labels', 'img_size'))
        if RANK in [-1, 0]:
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            # if i > 30: break  # stop when running 50 batchs in per epoch, only for faster testing
        ...
        if RANK in [-1, 0]:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _, error_list = val.run(data_dict,
...
def parse_opt(known=False):
    ...
    parser.add_argument('--hyp', type=str, default='data/hyps/hyp-p6.yaml', help='hyperparameters path')
